...
class LogisticRegression(object):
    ...
    def loss(self, X_batch, Y_batch):
        ...
        pred = sigmoid(X_batch.dot(self.weights))
        # calculate loss (stored in variable name loss) and gradients (stored in variable name grad)
        m = Y_batch.shape[0]
        loss = (-1/m) * np.sum(Y_batch.T*np.log(pred) + (1-Y_batch).T*np.log(1-pred))
        grad = -1*X_batch.T.dot((Y_batch-pred))
        ''' END YOUR CODE HERE '''
    ...
    def train(self,
        ...
        if self.add_bias:
            x = np.random.randn(X_train.shape[1])
            self.weights = np.append(x, 1)
        else:
            self.weights = np.random.randn(X_train.shape[1]) # Fill a correct value here
        ...
        # while taking in consideration the bias (so two cases according to the boolean value of add_bias).
        if self.add_bias:
            X_train_adj = np.hstack((X_train, np.ones((X_train.shape[0], 1))))  # Fill a correct value here
        else:
            X_train_adj = X_train
        ...
        for i in range(num_iters):
            ...
            self.weights = self.weights - lr*(grad + (reg_const/self.weights.shape[0])*self.weights)
