			...
			if pairs:
				...
				for word, pos_tag in zip(words_list, tags_list):
					...
					if words_count[word] == 1:
						emission_prob['<UNK>'][pos_tag] += 1
					else:
						emission_prob[word][pos_tag] += 1
					transition_prob[pos_tag][previous_tag] += 1
...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	def find_max_prob(t1, final_tag):
		...
		for i in range(len(t1)):
			temp = t1[i] * transition_prob[final_tag][pos_tags[i]]
			if temp > max_prob:
				max_prob = temp
				index = i
	...
	for sent in tqdm(test_words):
		...
		for index, tag in zip(range(tags_count), pos_tags):
			try:
				T1[index][0] = transition_prob[tag]['<s>'] * emission_prob[sent[0]][tag]
			except KeyError:
				T1[index][0] = transition_prob[tag]['<s>'] * emission_prob['<UNK>'][tag]
