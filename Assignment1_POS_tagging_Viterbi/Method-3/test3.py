...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	for sent in tqdm(test_words):
		...
		for index, tag in zip(range(tags_count), pos_tags):
			try:
				if transition_prob.loc['<s>', tag] == 0 and emission_prob[sent[0]][tag] > 0:
					transition_prob.loc['<s>', tag] = 0.00000001
				T1[index][0] = transition_prob.loc['<s>', tag] * emission_prob[sent[0]][tag]
			except KeyError:
				if transition_prob.loc['<s>', tag] == 0 and emission_prob['<UNK>'][tag] > 0:
					transition_prob.loc['<s>', tag] = 0.00000001
				T1[index][0] = transition_prob.loc['<s>', tag] * emission_prob['<UNK>'][tag]

		for pos in range(1, words_count):
			for index, tag in zip(range(tags_count), pos_tags):
				tag_index, max_prob = find_max_prob(T1[:,pos-1], tag)
				try:
					if max_prob == 0 and emission_prob[sent[pos]][tag] > 0:
						max_prob = 0.00000001
					T1[index, pos] = max_prob * emission_prob[sent[pos]][tag]
					T2[index, pos] = tag_index
				except KeyError:
					if max_prob == 0 and emission_prob['<UNK>'][tag] > 0:
						max_prob = 0.00000001
					T1[index, pos] = max_prob * emission_prob['<UNK>'][tag]
					T2[index, pos] = tag_index
