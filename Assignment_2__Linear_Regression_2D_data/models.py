...
class linear_model:
    ...
    def backward(self, y_actl, y_pred, feat):
        ...
        ### YOUR CODE SHOULD BEGIN HERE ###
        loss = y_pred - y_actl
        gradient = feat.T.dot(loss) / y_pred.shape[0]
    ...
    def loss(self, y_actl, y_pred):
        ...
        # print(y_actl.shape)
        #print("hi")
        a = np.sum((y_actl - y_pred)**2, axis=0)
        scalar_LOSS = np.asscalar(a) / (2*y_pred.shape[0])
        ### YOUR CODE SHOULD END HERE ###
...
def calc_features(X, choice, param=5):
    ...
    for x in X:
        if choice == 'linear':
            ...
            a = np.array(list(range(1, param+1))).reshape(param, 1)
            b = a.dot(x)
            si = np.sin(b)
            co = np.cos(b)
            c = np.append(si, co)
            ...
            # a = list(range(1, param))[::-1]
            # b = np.power(x, a)
            # a2 = np.array(list(range(1, param))).reshape(param-1, 1)
            # b2 = a2.dot(x)
            # si = np.sin(b2)
            # co = np.cos(b2)
            # c = np.append(b, si)
            ...
            # c_f = np.append(c, 1)
            # #print(c_f.shape)
            # feat.append(list(c_f))
            a = np.array(list(range(1, param+1))).reshape(param, 1)
            b = a.dot(x)
            si = 30*np.sin(np.cos(b))
            co = 30*np.cos(np.sin(b))
            c = np.append(si, co)
            a2 = list(range(param))[::-1]
            b2 = np.power(x, a2)
            c_f = np.append(c, b2)
            feat.append(list(c_f))
            # print(x)
